...
<%
	/*
		세션 생성과 활용
		1. 클라이언트(브라우저)의 최초 서버 대상 요청시 쿠키 베이스의
		    세션제너레이트 키를 서버로 전송(해당 클라이언트와 맵핑되는 세션이 X)
		2. 서버에 전송된 세션제너레이트 키를 취득하고 활용해서 세션을 생성하고,
		    생성된 세션에 아이디를 부여
		3. 클라이언트의 최초 요청시 신규 생성된 세션에서 세션아이디를 응답객체에
		    저장 후 클라이언트 대상 응답컨텐츠와 함께 전송
		4. 클라이언트는 이후 매번 서버대상 요청시 쿠키 베이스로 취득한 세션아이디를
		    함께 전송
		5. 클라이언트로부터 매번 요청시 전송되는 쿠키 베이스의 세션 아이디를
		    서버가 취득 후 세션아이디를 활용해서 해당 클라이언트와 맵핑되는
		    세션이 선택되고 활용
		6. 웹 프로그래밍 언어 별 쿠키베이스로 전송되는 세션 ID의 키가 상이
		    JSP : JSESSIONID
		    PHP : PHPSESSIONID
		    ASP : ASESSIONID
		...
		세션 유효시간 : Tomcat(default 30분)
					  서버단 자바를 활용해서 session.setMaxInactiveInterval(초단위 설정);
