...
<script>
	$(function(){
		$('input[type=image]').click(function(){
			...
			//쿼리스트링 전송방식 : POST
			//IE : 쿼리스트링 한글표현 문제로 에러가 발생
			//브라우저 주소창 직접 입력 : 주소?mem_name=encodeURIComponent('김은대')
			...
			//											 		  -URL 표현시	 활용되는 기호(: or / or ? or etc) 포함
			//encodeURI()	       :		decodeURI()  		  -유니코드 형식으로 인코딩 또는 디코딩 
			//											 		  -URL 표현시	 활용되는 기호(: or / or ? or etc) 불포함
			//encodeURIComponent() :		decodeURI Conponent() -유니코드 형식으로 인코딩 또는 디코딩 
			//											 		  -URL 표현을 배제한 단순 문자열 대상으로 인코딩과 디코딩 수행
			 ...
			 $.ajax({
				    ...
				    , success : function(result) {
				    			...
				    			var htmls = '<table width="200px", height="200px", border="0", cellspacing="0", cellpadding="0", style="overflow:scroll", id="zipcodeTBL">' +
				    						 '<thead>' +
				    						     '<tr>' +
				    						      '<th>우편번호</th>' +
				    						      '<th>시도</th>' +
